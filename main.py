...
try:
    ...
    You should refer to the user as "Mishu" or "Sir" in your responses.
...
def process_with_gemini(query):
    """Process the query using Gemini LLM if available."""
    if gemini:
        try:
...
def process_with_intent_model(query):
    """Process the query using the intent classification model."""
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # First check if it's a specific command we already handle
        if ('facebook' in query) or ('discord' in query) or ('whatsapp' in query) or ('instagram' in query):
            ...
            speak("Volume muted")
        elif ("calculator" in query) and ("open" in query):
            openApp(query)
        elif ("notepad" in query) and ("open" in query):
            openApp(query)
        elif ("paint" in query) and ("open" in query):
            openApp(query)
        elif ("calculator" in query) and ("close" in query):
            closeApp(query)
        elif ("notepad" in query) and ("close" in query):
            closeApp(query)
        elif ("paint" in query) and ("close" in query):
            closeApp(query)
        elif ("google" in query) and ("search" in query):
            browsing(query)
        elif ("system" in query) and ("condition" in query):
            condition()
        elif ("exit" in query) or ("quit" in query) or ("bye" in query) or ("goodbye" in query):
            speak("Goodbye, Mishu. Have a great day!")
            sys.exit()
        else:
            # Try to process with Gemini first
            gemini_response = process_with_gemini(query)
            
            if gemini_response:
                speak(gemini_response)
            else:
                # Fall back to intent model if Gemini fails or isn't available
                intent_response = process_with_intent_model(query)
